...
sha(MyModel, sp, data, Budget(epochs = 448), mode = GeometricSchedule, rate = 2, maximize = false)
sha(MyModel, sample(sp, 64), data, Budget(epochs = 600), mode = ConstantSchedule, rate = 2, maximize = false)
...
validate(KFold(x, 10)) do train
    prms = sha(MyModel, sp, FixedSplit(train, 0.8), Budget(epochs = 100), mode = GeometricSchedule, rate = 2, maximize = false)
    return fit!(MyModel(; prms...), train, epochs = 10)
end
