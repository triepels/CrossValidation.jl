...
brute_fit(MyModel, sp, data, args = (epochs = 100,), maximize = false)

brute(MyModel, sample(sp, 64), data, args = (epochs = 100,), maximize = false)
brute_fit(MyModel, sample(sp, 64), data, args = (epochs = 100,), maximize = false)

hc(MyModel, sp, data, args = (epochs = 100,), nstart = 10, k = 1, maximize = false)
hc_fit(MyModel, sp, data, args = (epochs = 100,), nstart = 10, k = 1, maximize = false)
...
validate(KFold(x, 10)) do train
    parm = hc(MyModel, sp, FixedSplit(train, 0.8), args = (epochs = 100,), nstart = 10, k = 1, maximize = false)
    return fit!(MyModel(; parm...), train, epochs = 10)
end
