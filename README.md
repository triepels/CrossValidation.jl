# Description
Provides simple and lightweight implementations for model validation and hyperparameter search in Julia. 
...
The `CrossValidation.jl` package works on the basis of two functions `predict` and `score` which the user needs to override. The `predict` function generates predictions by a fitted model on some data.
...
The `score` function 'scores' how well a model performs on some data. In case of supervised learning, this will be classification accuracy or similar score.
...
## Hyperparameter Search
The hyperparameters `a` and `b` of `MyClassifier` cannot be deduced directly from data. Instead, these parameters need to be tuned by a hyperparameter search. The `CrossValidation.jl` package allows to do this based on an exhaustive search in combination with various resample methods. Let us define the following grid:

```julia
search = ExhaustiveSearch(a=1:2, b=3:4)
```

Accordingly, we need to specify the resampling method and corresponding data that is used for the search. We will use k-fold cross-validation with 5 folds:
...
## Model Validation
It might be of interest to validate how well `MyModel` performs out of sample. We do this by performing model validation. The `CrossValidation.jl` allows to perform model validation based on various resample methods. Let us use leave-p-out with `p` set to 100:
